...
const SelectCategoryDropdown = ({
  ...
  const calculateParentCheckboxState = (children: CategoryMap): boolean | "indeterminate" => {
    const childIds = Object.keys(children);
    const selectedChildren = childIds.filter((id) => selectedCategories.includes(id));
            ...
            <div className="flex flex-col" key={category}>
              <div className="flex flex-row items-center gap-2 p-1 pl-4 hover:bg-gray-100 rounded">
                <Checkbox
                  checked={selectedCategories.includes(category)}
                  indeterminate={
                    calculateParentCheckboxState(relationShips[category].children) ===
                    "indeterminate"
                  }
                  onCheckedChange={(checked) =>
                    handleChange(checked, category, relationShips[category].children)
