...
const SelectCategoryDropdown = ({
  ...
  const RenderCheckBox = ({
    ...
    let allChildrenId = Object.keys(relationShips || {});
    let isAllChildrenSelected = true;
    for(let i=0; i<allChildrenId.length; i++){
      if(selectedCategories.includes(allChildrenId[i]) === false;){
        isAllChildrenSelected = false;
        break;
      } 
    }
    return <Checkbox checked={isAllChildrenSelected ||  (!relationShips?.children && selectedCategories.includes(category))} />;
  };
