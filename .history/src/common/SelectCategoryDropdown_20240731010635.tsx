...
const SelectCategoryDropdown = ({
  ...
  const RenderCheckBox = (relationShips: CategoryMap | null, category: string) => {
    return (
      <Checkbox
        checked={!relationShips?.[category]?.children && selectedCategories.includes(category)}
      />
    );
  };
          ...
          {Object.keys(relationShips)
            ...
            .map((category) => (
              <div key={category} className="flex flex-col">
                ...
                >
                  {/* <RenderCheckBox
                    relationShips={relationShips?.[category]?.children}
                    category={category}
                  /> */}
                  <Checkbox
                    checked={
